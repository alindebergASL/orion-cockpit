...
export function UserManagement({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div
        className="w-11/12 max-w-lg rounded-xl border border-th-border-strong bg-th-surface shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-th-border px-5 py-4">
          <h2 className="text-sm font-semibold text-th-text">User Management</h2>
        ...
        <div className="max-h-[28rem] overflow-y-auto p-5">
                ...
                <div key={u.id}>
                  <div className="flex items-center justify-between rounded-lg border border-th-border px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm text-th-text">{u.displayName}</p>
                        <p className="text-[11px] text-th-text-secondary">@{u.username}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className="flex items-center gap-1 text-[11px] text-th-text-secondary">
                        {u.role === 'admin' ? (
                      ...
                      <button
                        ...
                        className="rounded p-1 text-th-text-muted hover:text-cyan-400"
                        title="Edit user"
                      >
                      ...
                      <button
                        ...
                        className="rounded p-1 text-th-text-muted hover:text-amber-400"
                  ...
                  {/* Inline edit form */}
                  {editingId === u.id && (
                    <div className="mt-1 rounded-lg border border-th-border bg-th-elevated/50 p-3">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          <label className="mb-1 block text-[11px] text-th-text-secondary">Display Name</label>
                          <input
                            value={editDisplayName}
                            onChange={(e) => setEditDisplayName(e.target.value)}
                            className="w-full rounded-md border border-th-border-strong bg-th-input px-2.5 py-1.5 text-xs text-th-text outline-none focus:border-cyan-600"
                          />
                        </div>
                        <div>
                          <label className="mb-1 block text-[11px] text-th-text-secondary">Role</label>
                          <select
                            value={editRole}
                      ...
                      <div className="mt-3 flex justify-end gap-2">
                        <button
                          ...
                          className="rounded-md px-3 py-1.5 text-xs text-th-text-secondary hover:text-th-text"
                        >
                          Cancel
                        </button>
                        <button
                  ...
                  {passwordId === u.id && (
                    <div className="mt-1 rounded-lg border border-th-border bg-th-elevated/50 p-3">
                      <label className="mb-1 block text-[11px] text-th-text-secondary">New Password for @{u.username}</label>
                      <input
                        type="password"
                        value={newPassword}
          ...
          {/* Add user form */}
          {showForm ? (
            <form onSubmit={handleCreate} className="mt-4 space-y-3 rounded-lg border border-th-border p-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="mb-1 block text-[11px] text-th-text-secondary">Username</label>
                  <input
                    value={username}
                ...
                <div>
                  <label className="mb-1 block text-[11px] text-th-text-secondary">Password</label>
                  <input
                    type="password"
                    value={password}
            ...
            <button
              ...
              className="mt-4 flex w-full items-center justify-center gap-1.5 rounded-lg border border-dashed border-th-border-strong py-2.5 text-xs text-th-text-secondary hover:border-cyan-600 hover:text-cyan-400"
            >
