          ...
          <div key={i} className="flex flex-col overflow-hidden">
            ...
            <div
              className={`flex flex-col items-center border-b px-2 py-2 ${
                today ? 'border-cyan-600 bg-cyan-600/10' : 'border-slate-800'
              }`}
            >
              <span className="text-[11px] text-slate-500">{dayName(i)}</span>
              <span
            ...
            <div className="flex-1 space-y-1 overflow-y-auto p-1.5">
              {dayEvents.length === 0 && (
                <div className="py-4 text-center text-[10px] text-slate-700">
                  No events
                </div>
              )}
              {dayEvents.map((evt) => (
...
function EventBlock({
  ...
  return (
    ...
    >
      ...
      {event.location && (
        <div className="truncate text-[10px] text-slate-500">{event.location}</div>
      )}
    </button>
  );
}
